...
class SolanaManager(SolanaRPCMixin):
    ...
    def get_token_balances(self, account: SolanaAddress) -> dict[Asset, FVal]:
        ...
        """
        balances: dict[Asset, FVal] = defaultdict(lambda: ZERO)
        for token_program_id in (TOKEN_PROGRAM_ID, TOKEN_2022_PROGRAM_ID):
            ...
            for account_info in response.value:
                ...
                if decoded.amount == ZERO:
                    log.debug(f'Found token {token_address} with zero balance for {account}. Skipping.')  # noqa: E501
                    continue
                ...
                log.debug(f'Found {token} token balance for {account} with balance {amount}')
    ...
    def _get_metadata_from_extensions(
        ...
        if metadata_address == token_address:  # Token uses the TokenMetadata extension
            if (
                (raw_data := get_extension_data(extension_type=ExtensionType.TokenMetadata, tlv_data=mint_info.tlv_data)) is None or  # noqa: E501
