...
def trade_from_poloniex(poloniex_trade: dict[str, Any]) -> list[SwapEvent]:
    ...
    try:
        ...
        timestamp = deserialize_timestamp_ms_from_intms(poloniex_trade['createTime'])
        spend_asset, spend_amount, receive_asset, receive_amount = get_swap_spend_receive(
            raw_trade_type=poloniex_trade['side'],
            base_asset=asset_from_poloniex(get_pair_position_str(pair, 'first')),
    ...
    def _deserialize_trade(
        ...
        try:
            if account_type == 'SPOT':
                timestamp = deserialize_timestamp_from_intms(trade_data['createTime'])
