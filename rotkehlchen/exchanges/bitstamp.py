    ...
    def query_online_history_events(
        ...
        if start_ts != Timestamp(0):
            ...
            with self.db.conn.read_ctx() as cursor:
                query_result = history_db.get_history_events(
                    cursor=cursor,
                    filter_query=HistoryEventFilterQuery.make(
                        limit=1,
                        location=Location.BITSTAMP,
                        to_ts=start_ts,
                        order_by_rules=[('timestamp', False)],
                    ),
                    has_premium=True,
                )
                if (
                    len(query_result) != 0 and
                    (latest_movement := query_result[0]).extra_data is not None and
                    'movement_id' in latest_movement.extra_data
                ):
                    since_id = int(latest_movement.extra_data['movement_id']) + 1
        ...
        for asset_movement in asset_movements:
            assert asset_movement.extra_data is not None  # should always be set here
            for idx, crypto_movement in enumerate(crypto_asset_movements):
                assert crypto_movement.extra_data is not None  # should always be set here
                if (
            ...
            for idx, crypto_movement in enumerate(crypto_asset_movements):
                query_result = history_db.get_history_events(
                    ...
                    filter_query=HistoryEventFilterQuery.make(
                        limit=1,
                        location=Location.BITSTAMP,
                        to_ts=ts_ms_to_sec(crypto_movement.timestamp),
                        event_types=[crypto_movement.event_type],
                        event_subtypes=[crypto_movement.event_subtype],
                        assets=(crypto_movement.asset,),
                        order_by_rules=[('timestamp', False)],
                    ),
                    has_premium=True,
                )
                if len(query_result) != 0:
                    matched_movement = query_result[0]
                    extra_data = matched_movement.extra_data if matched_movement.extra_data is not None else {}  # noqa: E501
                    assert crypto_movement.extra_data is not None  # should always be set here
                    extra_data.update(crypto_movement.extra_data)
