...
class RestAPI:
    ...
    def _ensure_event_tx_existence(self, event: 'HistoryBaseEntry') -> Response | None:
        ...
        Returns an error response if unable to get the tx, otherwise returns None.
        """
        ...
        if event.location_label is None:
            return api_response(
                result=wrap_in_fail_result('The location_label must be set to pull the tx for the given hash from onchain.'),  # noqa: E501
        ...
        if blockchain.is_evmlike():
            if self.rotkehlchen.chains_aggregator.zksync_lite.query_single_transaction(
                ...
                concerning_address=string_to_evm_address(event.location_label),
            ) is not None:
                return None
        else:  # chain is EVM
            with suppress(KeyError, DeserializationError, RemoteError, AlreadyExists, InputError):
                self.rotkehlchen.chains_aggregator.get_chain_manager(
                    ...
                    tx_hash=event.tx_hash,
                    associated_address=string_to_evm_address(event.location_label),
                )
                return None
