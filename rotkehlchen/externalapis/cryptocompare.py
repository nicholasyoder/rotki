...
class Cryptocompare(
    ...
    def _api_query(
        ...
        while tries >= 0:
            log.debug('Querying cryptocompare', url=url.value, params=params)
            try:
                response = self.session.get(url.value, timeout=timeout, params=params)
            ...
            try:
                ...
                if json_ret.get('Response', 'Success') != 'Success':
                    error_message = f'Failed to query cryptocompare for: "{url.value}"'
